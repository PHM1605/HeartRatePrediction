...
class Sequence2Sequence():
    ...
    def build_model(self):
        ...
        # model for training
        encoder_input = Input((encoder_steps, 1)) # [None, 20, 1]
    ...
    def fit(self, train_df=None, data_formatter=None):
        ...
        data_input = data[:, :self.num_encoder_steps, 0]
        data_target = data[:, self.num_encoder_steps:, 0]
        self.model.fit(x=[data_input, data_target], y=np.pad(data_target, pad_width=[(0,0),(0,1)]), epochs=self.num_epochs, batch_size=self.minibatch_size, shuffle=True, verbose=1)
        
    def predict(self):
        data = TFTDataCache.get('test')
        inputs = data['inputs'][:, :self.num_encoder_steps, 0]
    ...
    def evaluate(self, y_true, y_pred):
        ...
        for quantile in quantiles:
            loss.append( utils.numpy_quantile_loss( y_true, y_pred, quantile) )
        return loss